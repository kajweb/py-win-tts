...
##													      		##
## 		  					PY-WIN-TTS						    ##
## Call SAPI.SpVoice pronunciation through Microsoft Speech SDK ##
## 		 	  If you need to output to the microphone, 	    	##
##   		please install the virtual sound card yourself	   	##
...
class pyWinTTS():
	...
	def getAudioOutputIndex( self ):
		...
		while audioOutput.Id != audioOutputLists[index].Id:
			...
			if( index>=audioOutputCount ):
				raise NotFoundAudioOutput( "Unable to match device" );
		return index;
	...
	def setAudioOutput( self, index ):
		...
		if not isExitsAudioOutput:
			raise NotFoundAudioOutput( "The device cannot be found" );
		audioOutputLists = self.getAudioOutputLists();
	...
	def getVoiceIndex( self ):
		...
		while voice.Id != voiceLists[index].Id:
			index += 1;
			if( index>=voiceCount ):
				raise NotFoundVoice( "Cannot match the announcer" );
		return index;
	...
	def setVoice( self, index ):
		...
		if not isExitsVoice:
			raise NotFoundVoice( "The announcer could not be found" );
		VoiceLists = self.getVoiceLists();
	...
	def setVolume( self, value ):
		...
		if not value>=0 and not value<=100:
			raise VolumeSettingError( "Sound output range is not in valid value" );
		spVoice.Volume = value;
		print( "[Success] Set the volume to %d" % value );
...
class RunTimeError(Exception):
	def __init__(self, errmsg):
		print( "【Execution exception】：%s"%errmsg );
		self.errmsg = errmsg
	def __str__(self):
		return self.errmsg
